...
class OrderCustomWebShopperFilterExtension implements QueryCollectionExtensionInterface
{
    ...
    public function applyToCollection(QueryBuilder $qb, QueryNameGeneratorInterface $queryNameGenerator, string $resourceClass, ?Operation $operation = null, array $context = []): void
    {
        ...
        $productAlias = $queryNameGenerator->generateJoinAlias('product');
        $qb->join("$rootAlias.product", $productAlias);

        $qb->andWhere("$productAlias.customerUuid = :current_user")
        ->setParameter(
            'current_user', 
